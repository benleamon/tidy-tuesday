...
interested_eps <- ranking_dialogue %>%
  group_by(book) %>%
...
ggplot(ranking_dialogue, aes(chapter_num, imdb_rating, color = book)) + 
  ...
  expand_limits(y=10)+
  facet_wrap(~book_num, labeller = book_names)+
  geom_text_repel(data = subset(ranking_dialogue, chapter %in% interested_eps$chapter), 
...
ggplot(dialogue_ordered, aes(n, word))+
    ...
    facet_wrap(~character, scales = "free")+
    scale_y_reordered()+
    labs(title = "Most common words by character",
       caption = "Data: appa | Plot: Ben Leamon")
